...
class MainActivity : AppCompatActivity() {
    ...
    fun login(view: View) {
        ...
        builder.setMessage("Email: ${et_user.text}\n Password: ${et_pass.text}")
            ?.setTitle("Logging in...")
        builder.create()?.show()
    }
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        if (requestCode == CREATE_ACCOUNT_REQUEST) {
            if (resultCode == Activity.RESULT_OK) {
                et_user.setText(data?.getStringExtra(EXTRA_USER_EMAIL).toString())
                et_pass.setText(data?.getStringExtra(EXTRA_USER_PASSWORD).toString())
            }
        }
    }
}